...
class RN(nn.Module):
	...
	def build(self, resize):
		...
		x, output = self.repnet(Variable(torch.randn((1, 3, resize, resize))))
		self.d = x.size(2)  # this is the size of repnet
		self.c = x.size(1)  # this is the channel of repnet, not general channel
		print('repnet size:', x.size())  # (1,64,15,15)
		...
		self.coord = torch.from_numpy(coord).float().view(self.d, self.d, 2).transpose(0, 2).transpose(1,
		                                                                                               2).contiguous()
	...
	def forward(self, input, input_y, query, query_y, train=True):
		...
		input_f = self.repnet(input_f)[0].view(batchsz, setsz, self.c, d,
		                                    d)  # c: raw image channel, self.c: repnet channel output
		query_f = self.repnet(query_f)[0].view(batchsz, querysz, self.c, d, d)
		...
		input_f = input_f.view(batchsz, setsz, self.c, d * d).unsqueeze(1).unsqueeze(4).expand(batchsz, querysz, setsz,
		                                                                                       self.c, d * d, d * d)
		# [b, querysz, 1, self.c, d*d] => [b, querysz, 1, self.c, d*d, 1] => [b, querysz, setsz, self.c, d*d, d*d]
		query_f = query_f.view(batchsz, querysz, self.c, d * d).unsqueeze(2).unsqueeze(5).expand(batchsz, querysz,
		                                                                                         setsz, self.c, d * d,
		                                                                                         d * d)
